#!/usr/bin/env ts-node

import { writeFileSync } from 'fs';
import { join } from 'path';
import { generateAllDynamicSitemaps } from '../utils/dynamicSitemapGenerator';

/**
 * Script to regenerate all sitemap files with the latest treatment routes
 * This ensures the Cheek Filler page and any other new treatment pages are included
 */
function regenerateSitemaps() {
  console.log('🚀 Regenerating sitemaps with updated treatment routes...');
  
  const sitemaps = generateAllDynamicSitemaps();
  const publicDir = join(process.cwd(), 'public');

  Object.entries(sitemaps).forEach(([filename, content]) => {
    const filepath = join(publicDir, filename);
    writeFileSync(filepath, content, 'utf8');
    console.log(`✅ Generated ${filename}`);
  });

  console.log('\n🎉 All sitemaps regenerated successfully!');
  console.log('\nGenerated files:');
  console.log('- sitemap.xml (main index)');
  console.log('- sitemap-pages.xml (main & company pages)');
  console.log('- sitemap-treatments.xml (treatment pages - AUTO-DETECTED)');
  console.log('- sitemap-blog.xml (blog posts)');
  console.log('- sitemap-locations.xml (location pages)');
  console.log('\n✅ Cheek Filler page is now included in the treatment sitemap!');
}

if (require.main === module) {
  regenerateSitemaps();
}

export { regenerateSitemaps };